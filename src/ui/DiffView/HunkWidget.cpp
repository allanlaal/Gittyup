...
void HunkWidget::load(git::Patch &staged, bool force)
{
  ...
  setEditorLineInfos(lines, linesStaged);
...
void HunkWidget::setEditorLineInfos(QList<Line>& lines, QList<Line>& linesStaged)
{
        ...
        switch (lines[lidx].origin()) {
          ...
          case GIT_DIFF_LINE_ADDITION: {
            ...
            if (linesStaged.count() > 0) {
                ...
                //QByteArray newLine = lines[lidx].newLine();
                // check if line is in staged lines
                Line lineStaged = linesStaged[lidx - offset - stagedDeletions]; // - (additions - stagedAdditions) + (deletions - stagedDeletions)];
                if (lineStaged.origin() == '+') {
                  stagedAdditions++;
                  staged = true;
                }
            ...
            // Check if staged
            if (linesStaged.count() > 0) {
                auto stagedLine = linesStaged[lidx - offset];
                QByteArray oldLine = lines[lidx].oldLine();
